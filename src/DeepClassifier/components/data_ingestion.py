...
class DataIngestion:
    ...
    def download_file(self):
        ...
        ):  ## To check if file is present or not
    ...
    def _get_updated_list_of_files(
        self, list_of_files
    ):  ## To get the list of files that ends with extension .jpg and inages of cat and dog
        return [
            f
    ...
    def _preprocess(self, zf: ZipFile, f: str, working_dir: str):
        ...
        if (
            os.path.getsize(target_filepath) == 0
        ):  ## To check if the file is having the size 0
            logger.info(
    ...
    def unzip_and_clean(self):  ## To unzip the file that we want to open
        ...
        with ZipFile(
            file=self.config.local_data_file, mode="r"
        ) as zf:  ##zf is zip file alias
