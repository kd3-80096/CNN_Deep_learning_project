...
class Test_DataIngestion:
    data_ingestion_config = DataIngestionConfig(
        ...
        local_data_file="tests/data/data_integration.zip",   ## saving as data_integration.zip
        unzip_dir="tests/data/")

    def test_download(self):
        data_ingestion = DataIngestion(config=self.data_ingestion_config)
        data_ingestion.download_file()  ## simply download the file
        assert os.path.exists(self.data_ingestion_config.local_data_file)

    def test_unzip(self):
        data_ingestion = DataIngestion(config=self.data_ingestion_config)
        data_ingestion.unzip_and_clean()  ## unzippig the file and cleaning
