...
class Test_read_yaml:
    yaml_files = [
        "tests/data/empty.yaml", ##\ is correct in a Windows file path 
        "tests/data/demo.yaml"
    ]

    def test_read_yaml_empty(self):
        with pytest.raises(ValueError):
            read_yaml(Path(self.yaml_files[0])) ## here [0] means we are passing the first file

    def test_read_yaml_return_type(self):
        respone = read_yaml(Path(self.yaml_files[-1])) ## here [-1] means we are passing the second file
